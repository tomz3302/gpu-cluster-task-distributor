...
# ---------------------------------------------------------------
# Configuration — paste your Colab public URL here
# ---------------------------------------------------------------

WORKER_BASE_URL = "http://hiidj-8-228-21-250.run.pinggy-free.link"
WORKER_URL = f"{WORKER_BASE_URL}/generate"
...
async def main():
    # Verify the worker is reachable before starting
    print(f"Connecting to worker at: {WORKER_BASE_URL}")
    try:
        async with httpx.AsyncClient() as client:
            health = await client.get(f"{WORKER_BASE_URL}/health", timeout=10)
            info = health.json()
            print(f"Worker online — model: {info.get('model')}, "
                  f"max_concurrent: {info.get('max_concurrent_inference')}")
    except Exception as e:
        print(f"ERROR: Could not reach worker — {e}")
        print("Make sure WORKER_BASE_URL is set to your Colab public URL and the notebook is running.")
        return
    ...
    for concurrency, total_requests in test_plan:
        ...
        await asyncio.sleep(5)   # let the GPU settle
